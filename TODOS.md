...
- **CheckMuralModal loading UI**: Checking phase shows user's uploaded image preview (`checkingPreviewUrl` from blob, revoked when leaving checking or closing modal), a subtle top-to-bottom scan-line animation over the thumbnail, and a pulsing magnifying-glass icon; copy "Checking your photo…" and "This may take a few seconds."; `role="status"` and `aria-label="Checking your photo"` for a11y.
